...
export default function EditProductPage() {
  ...
  const [form, setForm] = useState({
    ...
    stock: "0", categoryId: "", brandId: "", specs: "",
  })
  ...
  useEffect(() => {
    Promise.all([
      ...
      fetch(`/api/products?all=true`).then((r) => r.json()).then((data) => {
        ...
        const p = (data.products || []).find((x: any) => x.id === parseInt(id))
        if (p) {
          setForm({
            name: p.name, slug: p.slug, description: p.description || "",
            price: String(p.price), oldPrice: p.oldPrice ? String(p.oldPrice) : "",
            stock: String(p.stock), categoryId: String(p.categoryId), brandId: String(p.brandId),
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    let specsObj = {}
    try { if (form.specs.trim()) specsObj = JSON.parse(form.specs) } catch { /* empty */ }
    ...
    await fetch(`/api/products/${params.id}`, {
      ...
      body: JSON.stringify({
        ...
        stock: parseInt(form.stock), categoryId: parseInt(form.categoryId),
        brandId: parseInt(form.brandId), specs: specsObj,
      }),
    })
  ...
  return (
    <div>
      <h2 className="text-lg font-semibold mb-6">Редактировать товар</h2>
      <form onSubmit={handleSubmit} className="bg-white rounded-xl border border-gray-200 p-6 space-y-4 max-w-2xl">
        ...
        <div>
          ...
          <textarea value={form.specs} onChange={(e) => setForm({ ...form, specs: e.target.value })} className="input-field" rows={4} />
        </div>
